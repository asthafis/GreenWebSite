...
namespace WebGYM.Controllers
{
    ...
    public class RegisterMemberController : ControllerBase
    {
        ...
        {
            var userId = Convert.ToInt32(this.User.FindFirst(ClaimTypes.Name));
        ...
        public HttpResponseMessage Post([FromBody] MemberRegistrationViewModel member)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        var userId = this.User.FindFirst(ClaimTypes.Name);
           
                        var automember =  Mapper.Map<MemberRegistration>(member);
